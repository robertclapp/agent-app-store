...
The `registry.json` at the repo root is the **single source of truth** for the tool list — both the frontend and `exchange-api/app/registry.py` load from it.
...
1. **Never report "done" with failing tests.** After any code change, run the full test suite and fix every failure before reporting completion.
   - Python: `cd exchange-api && python -m pytest tests/ -v`
...
- **Path resolution with `__file__`:** Count the `.parent` chain carefully. `exchange-api/app/registry.py` needs exactly three `.parent` calls to reach the repo root (`app -> exchange-api -> root`). Verify with a quick `python -c` check before committing.
